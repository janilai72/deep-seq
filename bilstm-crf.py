...
#sequence_lengths = tf.reduce_sum(tf.sign(input_x), 1)
#
...
with tf.name_scope('bi-lstm'):
    ...
    print("fw_output:",fw_output.shape)
    print("bw_output:",bw_output.shape)
    ...
    contact_reshape=tf.reshape(contact,shape=[-1,2*hidden_nums],name='contact')#[batch_size*sentence_len,2*hidden_nums]
    ...
    p=tf.nn.relu(tf.matmul(contact_reshape,W)+b)#[batch_size*sentence_len,tag_nums]
    logit= tf.reshape(p,shape=[-1,sentence_len,tag_nums],name='omit_matrix')#[batch_size,sentence_len,tag_nums]


with tf.name_scope("crf"):
    print(logit.shape,labels.shape,sequence_lengths.shape)#(?, ?, 7) (?, 100) (?,)
